...
| Animations | **Motion** (Framer Motion) + **View Transitions API** | scroll-reveal، stagger، انتقالات صفحات؛ يحترم `prefers-reduced-motion` |
...
- **حركات وانتقالات سلسة** عبر Motion + View Transitions API (scroll-reveal، stagger، micro-interactions، عدّادات متحركة)، باستعمال transform/opacity فقط (GPU)، مع احترام `prefers-reduced-motion` وبلا إضرار بـ Core Web Vitals.
